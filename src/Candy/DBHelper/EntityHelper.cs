...
namespace Candy.DbHelper
{
	...
	internal static class EntityHelper
	{
		...
		const string _systemLoadSuffix = ".SystemLoad";
		/// <summary>
		/// 根据实体类获取所有字段数组, 有双引号
		/// </summary>
		...
		public static string[] GetFieldsFromStaticType(Type type)
		{
			...
			return _typeFieldsDict[string.Concat(type.FullName, _systemLoadSuffix)];
		}
		...
		/// <returns></returns>
		static string[] GetFieldsFromStaticType<T>() where T : ICandyDbModel
		{
		...
		{
			InitStaticTypesFields(type);
			return _typeFieldsDictNoSymbol[string.Concat(type.FullName, _systemLoadSuffix)];
		}
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		static string[] GetFieldsFromStaticTypeNoSymbol<T>() where T : ICandyDbModel
		{
		...
		static void InitStaticTypesFields(Type t)
		{
			...
			if (!t.GetInterfaces().Any(f => f == typeof(ICandyDbModel))) return;
			...
			{
				var key = string.Concat(type.FullName, _systemLoadSuffix);
		...
		static void InitStaticTypesFields<T>() where T : ICandyDbModel
		{
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public static DbTableAttribute GetDbTable<T>() where T : ICandyDbModel
		{
		...
		{
			InitStaticTypesFields(type);
			return string.Join(", ", _typeFieldsDict[string.Concat(type.FullName, _systemLoadSuffix)].Select(f => $"{alias}.{f}"));
		}
		...
		/// <param name="alias"></param>
		/// <returns></returns>
		public static string GetModelTypeFieldsString<T>(string alias) where T : ICandyDbModel
		{
		...
		{
			InitStaticTypesFields(type);
			return string.Join(", ", _typeFieldsDictNoSymbol[string.Concat(type.FullName, _systemLoadSuffix)].Select(f => $"{alias}.{f}"));
		}
		...
		/// <param name="alias"></param>
		/// <returns></returns>
		public static string GetModelTypeFieldsStringNoSymbol<T>(string alias) where T : ICandyDbModel
		{
			return GetModelTypeFieldsString(alias, typeof(T));
		}
		...
		internal static void GetAllFields<T>(Action<PropertyInfo> action) where T : ICandyDbModel
		{
