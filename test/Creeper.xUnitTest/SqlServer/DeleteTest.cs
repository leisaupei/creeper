...
namespace Creeper.xUnitTest.SqlServer
{
	public class DeleteTest : BaseTest, IDeleteTest
	{
		...
		{
			var id = Context.Select<IdenPkModel>().Max(a => a.Id);
			var affrows = Context.Delete<IdenPkModel>(a => a.Id == id);
		...
		{
			var info = Context.Select<IdenPkModel>().OrderByDescending(a => a.Id).FirstOrDefault();
		...
		{
			var info = Context.Select<IdenPkModel>().OrderByDescending(a => a.Id).Take(2).ToList();
